...
High-risk research execution (`--allow-static-blocked` or `--execute-custom-code`) additionally requires both `strace` and `prlimit`; if either is missing, execution is refused. `prlimit` supplies CPU, file-size, process-count, file-descriptor, core-dump and address-space limits. The default address-space ceiling is 24 GiB (`LAYERFAULT_BEHAVIOUR_MEMORY_MB` can override it for a dedicated lab). These controls reduce denial-of-service risk but are **not equivalent to a hardware VM**: Bubblewrap shares the host kernel, so active analysis of genuinely hostile code should still run on a disposable/dedicated lab host with no sensitive workloads.
