...
fn core(rule: &str) -> Option<RuleCore> {
    ...
    let item = match normalized.as_str() {
        ...
        "LF-NATIVE-WX-SECTION" => RuleCore { rule_id: "LF-NATIVE-WX-SECTION", title: "Writable and executable native binary section", meaning: "A native binary member or segment contains memory flags marked both writable and executable (WX).", remediation: "Inspect the native binary build options and disable WX sections/stack before loading." },
        "LF-NATIVE-RPATH" => RuleCore { rule_id: "LF-NATIVE-RPATH", title: "Native binary RPATH header", meaning: "A native executable or library contains explicit RPATH or RUNPATH search path headers.", remediation: "Verify that runtime dynamic library search paths are fixed and restricted to trusted system or package boundaries." },
        "LF-NATIVE-EXEC-CAPABILITY" => RuleCore { rule_id: "LF-NATIVE-EXEC-CAPABILITY", title: "Native binary process execution capability", meaning: "Static analysis identified imports for process execution functions (e.g. execve, system, CreateProcess).", remediation: "Review the native extension capability evidence and verify why process execution functions are imported." },
        "LF-NATIVE-NETWORK-CAPABILITY" => RuleCore { rule_id: "LF-NATIVE-NETWORK-CAPABILITY", title: "Native binary network capability", meaning: "Static analysis identified imports for network socket or HTTP functions (e.g. connect, send, WinHttpOpen).", remediation: "Review the native extension capability evidence and verify why network socket/HTTP functions are imported." },
        "LF-NATIVE-DYNAMIC-LOAD" => RuleCore { rule_id: "LF-NATIVE-DYNAMIC-LOAD", title: "Native binary dynamic loader capability", meaning: "Static analysis identified imports for dynamic library loading functions (e.g. dlopen, LoadLibrary, GetProcAddress).", remediation: "Review the native extension to ensure dynamic loading target paths are bounded and verified." },
        "LF-CORR-CUSTOM-LOADER-NATIVE" => RuleCore { rule_id: "LF-CORR-CUSTOM-LOADER-NATIVE", title: "Python custom loader to native binary capability chain", meaning: "Python model code loads a native library that possesses process execution or network capability imports.", remediation: "Inspect the Python loader invocation and target native library capabilities before admitting the model package." },
...
fn limitations(rule: &str) -> &'static str {
    match rule {
        ...
        "LF-NATIVE-WX-SECTION" => "Layerfault identified writable and executable memory flags statically. It did not execute the binary, and WX permissions may be caused by legacy linker settings.",
        "LF-NATIVE-RPATH" | "LF-NATIVE-DYNAMIC-LOAD" => "Static detection of RPATH or dynamic loader symbols is capability evidence. It does not prove that unverified libraries are loaded at runtime or that the path is exploitable.",
        "LF-NATIVE-EXEC-CAPABILITY" | "LF-NATIVE-NETWORK-CAPABILITY" => "Static detection of process or network symbol imports is capability evidence, not proof of malicious intent. Layerfault did not execute the native binary.",
        "LF-CORR-CUSTOM-LOADER-NATIVE" => "Layerfault correlated a Python loader call site with native library capabilities. It did not execute the model code or verify that the native function path is invoked automatically.",
