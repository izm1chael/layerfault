...
impl BehaviourLimits {
    pub fn for_profile(profile: &str) -> Result<Self> {
        match profile.to_ascii_lowercase().as_str() {
            "quick" => Ok(Self { max_prompts: 8, max_turns: 2, max_tokens: 256, max_output_bytes: 128*1024, timeout_seconds: 90, max_mutations: 0, repeat_count: 1 }),
            "standard" => Ok(Self { max_prompts: 64, max_turns: 4, max_tokens: 512, max_output_bytes: 256*1024, timeout_seconds: 120, max_mutations: 32, repeat_count: 1 }),
            "deep" => Ok(Self { max_prompts: 256, max_turns: 6, max_tokens: 768, max_output_bytes: 512*1024, timeout_seconds: 180, max_mutations: 256, repeat_count: 2 }),
            "research" => Ok(Self { max_prompts: 1000, max_turns: 8, max_tokens: 1024, max_output_bytes: 1024*1024, timeout_seconds: 300, max_mutations: 4096, repeat_count: 3 }),
            other => bail!("unsupported review/behaviour profile '{other}'"),
        }
    }

    pub fn clamp(mut self, prompts: usize, turns: usize, tokens: u64, timeout: u64, mutations: usize, repeats: usize) -> Self {
        if prompts > 0 { self.max_prompts = self.max_prompts.min(prompts); }
        if turns > 0 { self.max_turns = self.max_turns.min(turns); }
        if tokens > 0 { self.max_tokens = self.max_tokens.min(tokens); }
        if timeout > 0 { self.timeout_seconds = self.timeout_seconds.min(timeout); }
        if mutations > 0 { self.max_mutations = self.max_mutations.min(mutations); }
        if repeats > 0 { self.repeat_count = self.repeat_count.min(repeats); }
        self
    }
}
...
pub fn replay_manifest(report: &BehaviourReport, probe_suite_path: Option<&Path>) -> BehaviourReplayManifest {
...
pub fn load_replay(path: &Path) -> Result<BehaviourReplayManifest> {
    ...
    if manifest.version != 1 { bail!("unsupported behaviour replay manifest version {}", manifest.version); }
    Ok(manifest)
}

pub fn run_external_llama(model: &Path, runtime_path: Option<&Path>, suite_path: Option<&Path>, seed: u64, limits: BehaviourLimits) -> Result<BehaviourReport> {
    static_admit(model)?;
    ...
    let executable = match runtime_path {
        ...
        None => crate::sources::find_executable("llama-cli").or_else(|| crate::sources::find_executable("main")).ok_or_else(|| anyhow::anyhow!("llama.cpp CLI was not found on PATH"))?,
    };
    ...
    for probe in suite.probes.iter().take(limits.max_prompts) {
        ...
        for repeat_index in 0..repeat {
            ...
            let result = runtime.run(&model, &combined, seed.saturating_add(repeat_index as u64), limits.max_tokens)?;
            let evaluation = evaluate::evaluate(&probe.category, &result.stdout, &[&canary_a, &canary_b]);
            executions.push(ProbeExecution {
                probe_id: probe.id.clone(), category: probe.category.clone(),
                prompt_sha256: sha256(combined.as_bytes()), response_sha256: sha256(result.stdout.as_bytes()),
                response_excerpt: bounded_excerpt(&result.stdout, 4096), duration_ms: result.duration_ms,
                exit_code: result.exit_code, timed_out: result.timed_out, evaluation,
            });
        }
    }
    let mut findings: Vec<String> = executions.iter().flat_map(|v| v.evaluation.rule_ids.clone()).collect();
    findings.sort(); findings.dedup();
    let high = executions.iter().any(|v| v.evaluation.risk == evaluate::Risk::High);
    let suspicious = executions.iter().any(|v| matches!(v.evaluation.risk, evaluate::Risk::Medium | evaluate::Risk::High));
    Ok(BehaviourReport {
...
pub fn compare_external_llama(base: &Path, derived: &Path, runtime_path: Option<&Path>, suite_path: Option<&Path>, seed: u64, limits: BehaviourLimits) -> Result<DifferentialReport> {
    ...
    let base_map: std::collections::BTreeMap<_, _> = base_report.executions.iter().map(|v| ((v.probe_id.as_str(), v.category.as_str()), v)).collect();
    let mut overall = crate::transformation::DifferentialBehaviourState::Expected;
    for d in &derived_report.executions {
        let Some(b) = base_map.get(&(d.probe_id.as_str(), d.category.as_str())) else { continue; };
        let class = evaluate::classify_difference(&b.evaluation, &d.evaluation);
        overall = evaluate::stronger_difference(overall, class);
        let mut rules = Vec::new();
        if matches!(class, crate::transformation::DifferentialBehaviourState::SecurityRegression | crate::transformation::DifferentialBehaviourState::SuspiciousTrigger | crate::transformation::DifferentialBehaviourState::HighRiskBehaviour) {
            rules.push("LF-DIFF-SECURITY-REGRESSION".to_owned());
            findings.push("LF-DIFF-SECURITY-REGRESSION".to_owned());
        }
        rows.push(DifferentialRow { probe_id: d.probe_id.clone(), category: d.category.clone(), base_risk: b.evaluation.risk.as_str().to_owned(), derived_risk: d.evaluation.risk.as_str().to_owned(), classification: class, rule_ids: rules });
    }
    findings.sort(); findings.dedup();
    Ok(DifferentialReport { schema_version: "1.0".to_owned(), base: base_report, derived: derived_report, rows, state: overall, findings })
}

fn static_admit(path: &Path) -> Result<()> {
    if path.is_dir() {
        let report = crate::package::inspect(path)?;
        if report.blocking() { bail!("static admission blocked package '{}'; behaviour was not run", path.display()); }
    } else {
        let report = crate::formats::artifact::inspect(path, crate::formats::artifact::ArtifactScanMode::Full)?;
        if report.blocking() { bail!("static admission blocked artifact '{}'; behaviour was not run", path.display()); }
    }
    Ok(())
}
...
fn resolve_gguf(path: &Path) -> Result<PathBuf> {
    if path.is_file() {
        ...
        if snap.format != "gguf" { bail!("llama.cpp behavioural backend requires a GGUF artifact, got {}", snap.format); }
        return Ok(path.to_path_buf());
    }
    let report = crate::package::inspect(path)?;
    let ggufs: Vec<_> = report.files.iter().filter(|v| v.relative_path.to_ascii_lowercase().ends_with(".gguf")).collect();
    if ggufs.len() != 1 { bail!("llama.cpp behavioural package must contain exactly one GGUF artifact; found {}", ggufs.len()); }
    Ok(PathBuf::from(report.root).join(&ggufs[0].relative_path))
}

fn synthetic_canary(identity: &str, seed: u64, label: &str) -> String {
    let digest = Sha256::digest(format!("layerfault-canary\0{identity}\0{seed}\0{label}").as_bytes());
    format!("LF_CANARY_{label}_{}", &hex::encode(digest)[..24])
}
fn sha256(bytes: &[u8]) -> String { format!("sha256:{}", hex::encode(Sha256::digest(bytes))) }
fn bounded_excerpt(value: &str, cap: usize) -> String { if value.len() <= cap { return value.to_owned(); } let mut end=cap; while end>0 && !value.is_char_boundary(end) {end-=1;} format!("{}…", &value[..end]) }
...
pub fn run_embedded(
    ...
    let embedded_identity = embedded_identity.ok_or_else(|| anyhow::anyhow!("no embedded probes executed"))?;
    ...
    finalize_report(model_identity, model.display().to_string(), runtime, suite, seed, limits, executions)
}
...
fn finalize_report(
    ...
    let suspicious = executions.iter().any(|value| {
        matches!(value.evaluation.risk, evaluate::Risk::Medium | evaluate::Risk::High)
    });
    Ok(BehaviourReport {
...
fn differential_from_reports(
    ...
    for derived in &derived_report.executions {
        let Some(base) = base_map.get(&(derived.probe_id.as_str(), derived.category.as_str())) else {
            continue;
        };
