...
use crate::transformation::{DerivedIntegrityState, LineageState, TransformationManifest, TransformationType};
use anyhow::Result;
...
pub fn compare_paths(base: &std::path::Path, derived: &std::path::Path, claim: Option<TransformationType>, manifest: Option<&TransformationManifest>) -> Result<ComparisonReport> {
...
pub fn compare_snapshots(base: ModelSnapshot, derived: ModelSnapshot, claim: Option<TransformationType>, manifest: Option<&TransformationManifest>) -> ComparisonReport {
    let mut findings = Vec::new();
    let mut components = BTreeMap::new();
    let tensors = compare_tensors(&base.tensors, &derived.tensors);

    component(&mut components, "architecture", &base.architecture.architecture, &derived.architecture.architecture);
    component(&mut components, "layer_count", &base.architecture.layer_count, &derived.architecture.layer_count);
    component(&mut components, "hidden_size", &base.architecture.hidden_size, &derived.architecture.hidden_size);
    component(&mut components, "attention_heads", &base.architecture.attention_heads, &derived.architecture.attention_heads);
    component(&mut components, "kv_heads", &base.architecture.kv_heads, &derived.architecture.kv_heads);
    component(&mut components, "vocabulary_size", &base.architecture.vocabulary_size, &derived.architecture.vocabulary_size);
    component(&mut components, "context_length", &base.architecture.context_length, &derived.architecture.context_length);
    component(&mut components, "tokenizer", &base.tokenizer, &derived.tokenizer);
    component(&mut components, "chat_template", &base.template, &derived.template);
    component(&mut components, "generation_config", &base.generation, &derived.generation);
    ...
    if let Some(claim) = claim {
        match claim {
            TransformationType::Quantization => {
                if tokenizer_changed {
                    contradicted = true;
                    findings.push(claim_contradiction("LF-LINEAGE-QUANTIZATION-TOKENIZER", "Quantization-only claim changed the tokenizer"));
                }
                if template_changed {
                    contradicted = true;
                    findings.push(claim_contradiction("LF-LINEAGE-QUANTIZATION-TEMPLATE", "Quantization-only claim changed the chat template"));
                }
                if !tensors.added.is_empty() || !tensors.removed.is_empty() || !tensors.shape_changed.is_empty() {
                    contradicted = true;
                }
            ...
            TransformationType::Repackaging => {
                if base.tensor_schema_hash != derived.tensor_schema_hash || tokenizer_changed || template_changed {
                    contradicted = true;
                    findings.push(claim_contradiction("LF-LINEAGE-REPACKAGE-CONTENT", "Repackaging claim changed model-semantic components"));
                }
            }
            TransformationType::TokenizerModification => {
                if !tokenizer_changed { findings.push(finding("LF-LINEAGE-CLAIM-NO-EVIDENCE", "lineage", "WARN", "MEDIUM", "Claimed tokenizer modification was not observed", "The normalized tokenizer summaries are identical.", "A transformation claim without corresponding evidence cannot be verified.", vec![], "The manifest or supplied artifacts may not match.", "Verify the exact child artifact and transformation record.")); }
            }
            TransformationType::TemplateModification => {
                if !template_changed { findings.push(finding("LF-LINEAGE-CLAIM-NO-EVIDENCE", "lineage", "WARN", "MEDIUM", "Claimed template modification was not observed", "The normalized chat-template summaries are identical.", "A transformation claim without corresponding evidence cannot be verified.", vec![], "The manifest or supplied artifacts may not match.", "Verify the exact child artifact and transformation record.")); }
            }
            TransformationType::FineTune | TransformationType::Conversion | TransformationType::Other => {}
        }
    }

    if let Some(manifest) = manifest {
        if manifest.parent.identity != base.identity.canonical || manifest.child.identity != derived.identity.canonical {
            contradicted = true;
        ...
        if claim.is_some_and(|claim| claim != manifest.transformation.kind) {
            contradicted = true;
            findings.push(claim_contradiction("LF-LINEAGE-MANIFEST-CLAIM", "CLI transformation claim conflicts with transformation manifest"));
        }
    }
    ...
    } else if arch_core_changed || base.architecture.architecture.is_none() || derived.architecture.architecture.is_none() {
    ...
    findings.sort_by(|a, b| a.rule_id.cmp(&b.rule_id).then_with(|| a.title.cmp(&b.title)));
    ComparisonReport { schema_version: "1.0".to_owned(), base, derived, claim, lineage, derived_integrity, findings, component_changes: components, tensor_changes: tensors }
}
...
fn compare_tensors(base: &[TensorSummary], derived: &[TensorSummary]) -> TensorChangeSummary {
    ...
    let mut out = TensorChangeSummary { base_count: base.len(), derived_count: derived.len(), ..Default::default() };
    for name in names {
        match (left.get(name), right.get(name)) {
            (None, Some(_)) => out.added.push(name.to_owned()),
            (Some(_), None) => out.removed.push(name.to_owned()),
            (Some(a), Some(b)) => {
                if a.shape != b.shape { out.shape_changed.push(name.to_owned()); }
                if a.dtype != b.dtype { out.dtype_changed.push(name.to_owned()); }
            }
            (None, None) => {}
        }
    }
    out.schema_identical = out.added.is_empty() && out.removed.is_empty() && out.shape_changed.is_empty();
    out
}

fn component<T: Serialize + PartialEq>(out: &mut BTreeMap<String, Value>, name: &str, base: &T, derived: &T) {
    out.insert(name.to_owned(), json!({"changed": base != derived, "base": base, "derived": derived}));
}
...
fn finding(rule_id: &str, domain: &str, status: &str, confidence: &str, title: &str, what_changed: &str, why: &str, evidence: Vec<String>, impact: &str, recommendation: &str) -> ComparisonFinding {
    ComparisonFinding { rule_id: rule_id.to_owned(), domain: domain.to_owned(), status: status.to_owned(), confidence: confidence.to_owned(), title: title.to_owned(), what_changed: what_changed.to_owned(), why_security_relevant: why.to_owned(), evidence, potential_impact: impact.to_owned(), recommendation: recommendation.to_owned() }
}
