...
pub fn from_json(value: &serde_json::Value) -> Vec<String> {
...
pub fn from_txt(text: &str) -> Vec<String> {
    text.lines()
        .filter(|line| !line.is_empty() && line.len() <= MAX_TOKEN_LEN)
        .take(MAX_VOCABULARY_ENTRIES)
        .map(str::to_owned)
        .collect()
}
...
mod tests {
    ...
    #[test]
    fn parses_vocab_txt_lines() {
        let text = "hello\nworld\n<|im_start|>\n";
        let tokens = from_txt(text);
        assert_eq!(tokens, vec!["hello", "world", "<|im_start|>"]);
    }

    #[test]
    fn empty_lines_are_skipped() {
        let text = "hello\n\nworld\n";
        assert_eq!(from_txt(text), vec!["hello", "world"]);
    }
