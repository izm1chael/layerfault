...
impl PlatformDb {
    ...
    pub fn list_findings(&mut self,limit:usize)->Result<Vec<serde_json::Value>>{
        ...
        match self{
            Self::Sqlite(c)=>{let mut st=c.prepare("SELECT id,review_id,rule_id,domain,status,confidence,detail_json,created_at FROM findings ORDER BY created_at DESC LIMIT ?1")?;let rows=st.query_map(rusqlite::params![limit],|r|Ok((r.get::<_,String>(0)?,r.get::<_,String>(1)?,r.get::<_,String>(2)?,r.get::<_,String>(3)?,r.get::<_,String>(4)?,r.get::<_,String>(5)?,r.get::<_,String>(6)?,r.get::<_,i64>(7)?)))?.collect::<std::result::Result<Vec<_>,_>>()?;rows.into_iter().map(|(id,review,rule,domain,status,confidence,detail,created)|Ok(serde_json::json!({"id":id,"review_id":review,"rule_id":rule,"domain":domain,"status":status,"confidence":confidence,"detail":serde_json::from_str::<serde_json::Value>(&detail).unwrap_or_else(|_|serde_json::Value::String(detail)),"created_at":created}))).collect()},
            Self::Postgres(c)=>c.query("SELECT id,review_id,rule_id,domain,status,confidence,detail_json,created_at FROM findings ORDER BY created_at DESC LIMIT $1",&[&limit])?.into_iter().map(|r|{let detail:String=r.get(6);Ok(serde_json::json!({"id":r.get::<_,String>(0),"review_id":r.get::<_,String>(1),"rule_id":r.get::<_,String>(2),"domain":r.get::<_,String>(3),"status":r.get::<_,String>(4),"confidence":r.get::<_,String>(5),"detail":serde_json::from_str::<serde_json::Value>(&detail).unwrap_or_else(|_|serde_json::Value::String(detail)),"created_at":r.get::<_,i64>(7)}))}).collect()
        }
    }
