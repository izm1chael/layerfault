...
impl PlatformDb {
    ...
    pub fn succeed(&mut self, id: &str) -> Result<()> {
        self.finish(id, "SUCCEEDED", None)
    }
    pub fn fail(&mut self, id: &str, attempts: i64, max_attempts: i64, error: &str) -> Result<()> {
        let state = if attempts >= max_attempts {
            "DEAD"
        } else {
            "READY"
        };
        self.finish(id, state, Some(error))
    }
    fn finish(&mut self, id: &str, state: &str, error: Option<&str>) -> Result<()> {
        let now = now_i64();
        let error = error.map(|v| truncate(v, 8192));
        match self {
            Self::Sqlite(c) => {
                c.execute("UPDATE jobs SET state=?1,lease_owner=NULL,lease_until=NULL,last_error=?2,finished_at=CASE WHEN ?1='SUCCEEDED' OR ?1='DEAD' THEN ?3 ELSE NULL END WHERE id=?4",rusqlite::params![state,error,now,id])?;
            }
            Self::Postgres(c) => {
                c.execute("UPDATE jobs SET state=$1,lease_owner=NULL,lease_until=NULL,last_error=$2,finished_at=CASE WHEN $1='SUCCEEDED' OR $1='DEAD' THEN $3 ELSE NULL END WHERE id=$4",&[&state,&error,&now,&id])?;
            }
        }
        Ok(())
    }
    ...
    pub fn upsert_revision(
        ...
        match self {
            Self::Sqlite(c) => {
                ...
                c.execute("INSERT INTO model_revisions(id,model_id,revision,observed_at,metadata_json) VALUES(?1,?2,?3,?4,?5) ON CONFLICT(model_id,revision) DO UPDATE SET observed_at=excluded.observed_at,metadata_json=excluded.metadata_json",rusqlite::params![revision_id,model_id,revision,now,metadata])?;
            }
            Self::Postgres(c) => {
                c.execute("INSERT INTO models(id,canonical_name,created_at) VALUES($1,$2,$3) ON CONFLICT(id) DO NOTHING",&[&model_id,&repo,&now])?;
                c.execute("INSERT INTO model_revisions(id,model_id,revision,observed_at,metadata_json) VALUES($1,$2,$3,$4,$5) ON CONFLICT(model_id,revision) DO UPDATE SET observed_at=EXCLUDED.observed_at,metadata_json=EXCLUDED.metadata_json",&[&revision_id,&model_id,&revision,&now,&metadata])?;
            }
        }
        Ok(revision_id)
    }
    ...
    pub fn insert_review(
        ...
        match self {
            Self::Sqlite(c) => {
                c.execute("INSERT OR IGNORE INTO reviews(id,revision_id,final_decision,created_at,body_json) VALUES(?1,?2,?3,?4,?5)",rusqlite::params![id,revision_id,decision,now,body_text])?;
            }
            Self::Postgres(c) => {
                c.execute("INSERT INTO reviews(id,revision_id,final_decision,created_at,body_json) VALUES($1,$2,$3,$4,$5) ON CONFLICT(id) DO NOTHING",&[&id,&revision_id,&decision,&now,&body_text])?;
            }
        }
    ...
    fn index_review_findings(
        ...
        for (index, value) in values.into_iter().enumerate() {
            let rule = value
                .get("rule_id")
                .and_then(|v| v.as_str())
                .or_else(|| value.get("rule").and_then(|v| v.as_str()))
            ...
            let finding_id = stable_id(
                "lffinding",
                &[review_id.as_bytes(), rule.as_bytes(), idx.as_bytes()],
            );
            match self {
                Self::Sqlite(c) => {
                    c.execute("INSERT OR REPLACE INTO findings(id,review_id,rule_id,domain,status,confidence,detail_json,created_at) VALUES(?1,?2,?3,?4,?5,?6,?7,?8)",rusqlite::params![finding_id,review_id,rule,domain,status,confidence,detail,created_at])?;
                }
                Self::Postgres(c) => {
        ...
        if body.get("final_decision").and_then(|v| v.as_str()) == Some("BLOCK") {
            let title = format!("Blocking Layerfault review {review_id}");
            let advisory_body = serde_json::json!({"review_id":review_id,"summary":"A pinned model revision produced blocking Layerfault findings. Review the underlying evidence before deployment."});
            let text = serde_json::to_string(&advisory_body)?;
            let advisory_id = stable_id("lfadvisory", &[review_id.as_bytes()]);
            match self {
                Self::Sqlite(c) => {
                    c.execute("INSERT OR IGNORE INTO advisories(id,review_id,title,severity,body_json,created_at) VALUES(?1,?2,?3,'HIGH',?4,?5)",rusqlite::params![advisory_id,review_id,title,text,created_at])?;
                }
                Self::Postgres(c) => {
