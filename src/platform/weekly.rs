...
pub fn render(review: &WeeklyReview, public_base: Option<&str>) -> NewsletterBodies {
    let models = n(&review.counts, "models");
    let revisions = n(&review.counts, "revisions");
    let reviews = n(&review.counts, "reviews");
    let blocks = n(&review.counts, "blocking_reviews");
    let warnings = n(&review.counts, "warning_reviews");
    let no_findings = n(&review.counts, "no_block_or_warning_reviews");
...
pub fn send_smtp(
    ...
    let message = Message::builder()
        .from(from.parse().context("invalid newsletter From address")?)
        .to(to.parse().context("invalid newsletter To address")?)
        .subject(&bodies.subject)
        .header(lettre::message::header::ContentType::TEXT_HTML)
        .body(bodies.html.clone())?;
    ...
    mailer
        .send(&message)
        .context("SMTP newsletter send failed")?;
    Ok(())
}
