...
pub fn build_snapshot(path: &Path) -> Result<ModelSnapshot> {
    ...
    if metadata.is_dir() {
        ...
        bail!(
            "model target '{}' is not a regular file or directory",
            path.display()
        )
    }
}
...
fn merge_architecture(target: &mut ArchitectureSummary, child: ArchitectureSummary) {
    if target.architecture.is_none() {
        target.architecture = child.architecture;
    }
    if target.layer_count.is_none() {
        target.layer_count = child.layer_count;
    }
    if target.hidden_size.is_none() {
        target.hidden_size = child.hidden_size;
    }
    if target.attention_heads.is_none() {
        target.attention_heads = child.attention_heads;
    }
    if target.kv_heads.is_none() {
        target.kv_heads = child.kv_heads;
    }
    if target.vocabulary_size.is_none() {
        target.vocabulary_size = child.vocabulary_size;
    }
    if target.context_length.is_none() {
        target.context_length = child.context_length;
    }
        ...
        if key.starts_with("tokenizer.") {
            ...
            if key.ends_with("_token_id") {
                if let Some(id) = value
                    .signed_value
                    .or_else(|| value.unsigned_value.and_then(|v| i64::try_from(v).ok()))
                {
...
fn architecture_from_config(config: Option<&Value>) -> ArchitectureSummary {
    let Some(config) = config.and_then(Value::as_object) else {
        return ArchitectureSummary::default();
    };
    ...
    for (key, value) in config {
        ...
        if lower.contains("rope") {
            rope.insert(key.clone(), value.clone());
        }
        if lower.contains("norm") || lower.contains("rms_norm") {
            normalization.insert(key.clone(), value.clone());
        }
    }
    ArchitectureSummary {
        architecture,
...
fn tokenizer_from_package(
    root: &Path,
    tokenizer: Option<&Value>,
    config: Option<&Value>,
    special: Option<&Value>,
) -> Result<Option<TokenizerSummary>> {
    ...
    if tokenizer.is_none() && config.is_none() && special.is_none() && tokenizer_hash.is_none() {
        return Ok(None);
    }
    let mut summary = TokenizerSummary {
        tokenizer_file_hash: tokenizer_hash,
        tokenizer_config_hash: config_hash,
        ..Default::default()
    };
    if let Some(tokenizer) = tokenizer {
        ...
        if let Some(added) = tokenizer.get("added_tokens") {
            summary.added_tokens_hash = Some(hash_json(added)?);
        }
    }
...
fn generation_from_package(
    root: &Path,
    value: Option<&Value>,
) -> Result<Option<GenerationConfigSummary>> {
...
fn metadata_entry_value(value: &gguf::GgufMetadataEntry) -> Value {
    ...
    if let Some(v) = value.unsigned_value {
        return Value::from(v);
    }
    if let Some(v) = value.signed_value {
        return Value::from(v);
    }
...
fn read_bounded(path: &Path, cap: u64) -> Result<Vec<u8>> {
    ...
    if len > cap {
        bail!(
            "'{}' is {len} bytes, above metadata cap {cap}",
            path.display()
        );
    }
    let reader = file;
    let mut bytes =
        Vec::with_capacity(usize::try_from(len).context("metadata size does not fit usize")?);
