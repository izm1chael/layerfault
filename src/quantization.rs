...
fn run_tool(exe:&Path,input:&Path,output:&Path,quantization:&str,timeout:u64)->Result<()> {
    ...
    let start=Instant::now();let status=loop{if let Some(s)=child.try_wait()?{break s;}if start.elapsed()>=Duration::from_secs(timeout){let _=child.kill();let _=child.wait();bail!("quantizer timed out after {timeout} seconds");}std::thread::sleep(Duration::from_millis(25));};
...
fn read_capped<R:Read>(mut reader:R)->Result<Vec<u8>>{let mut out=Vec::new();let mut buf=[0u8;8192];loop{let n=reader.read(&mut buf)?;if n==0{break;}if out.len().saturating_add(n)>MAX_TOOL_OUTPUT{bail!("tool output exceeded {MAX_TOOL_OUTPUT} byte cap");}out.extend_from_slice(&buf[..n]);}Ok(out)}
...
fn bounded_version(path:&Path)->Option<String>{let mut child=std::process::Command::new(path).arg("--version").env_clear().stdout(Stdio::piped()).stderr(Stdio::null()).spawn().ok()?;let stdout=child.stdout.take()?;let (tx,rx)=mpsc::channel();std::thread::spawn(move||{let _=tx.send(read_capped(stdout));});let start=Instant::now();loop{if child.try_wait().ok().flatten().is_some(){break;}if start.elapsed()>Duration::from_secs(5){let _=child.kill();let _=child.wait();return None;}std::thread::sleep(Duration::from_millis(20));}let bytes=rx.recv_timeout(Duration::from_secs(1)).ok()?.ok()?;let text=String::from_utf8_lossy(&bytes).trim().to_owned();if text.is_empty(){None}else{Some(text.chars().take(4096).collect())}}
