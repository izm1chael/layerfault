...
pub fn parse_elf(file: &File, file_len: u64, offset: u64) -> Result<Option<BinaryMetadata>> {
    ...
    if phnum > 0 && phentsize >= if is_64 { 56 } else { 32 } {
        ...
        if let Some(ph_data) = super::read_at(file, offset + phoff, ph_table_bytes)? {
            for chunk in ph_data.chunks_exact(phentsize as usize) {
                ...
                if p_type == 3 && interpreter.is_none() && p_filesz > 0 && p_filesz <= 4096 {
                    ...
                    if let Some(interp_bytes) =
                        super::read_at(file, offset + p_offset, p_filesz as usize)?
                    {
    ...
    if shnum > 0 && shentsize >= if is_64 { 64 } else { 40 } {
        let shstr_offset = if shstrndx < shnum {
            let shstr_entry_offset = offset + shoff + (shstrndx * shentsize);
        ...
        let shstr_table = if shstr_offset > 0 {
            super::read_at(file, offset + shstr_offset, MAX_STRING_BYTES)?.unwrap_or_default()
        } else {
            Vec::new()
        };
        ...
        if let Some(sh_data) =
            super::read_at(file, offset + shoff, (scan_shnum * shentsize) as usize)?
        {
    ...
    if let Some((dyn_off, dyn_sz)) = dynamic_phdr {
        ...
        if let Some(dyn_bytes) = super::read_at(file, offset + dyn_off, dyn_sz as usize)? {
            ...
            let strtab_bytes = if let Some(off) = strtab_offset {
                ...
                super::read_at(file, absolute, MAX_STRING_BYTES.min(available))?.unwrap_or_default()
            } else {
                Vec::new()
            };
            ...
            if let Some(sym_off) = symtab_offset {
                ...
                if let Some(sym_data) = super::read_at(file, absolute, requested.min(available))? {
