...
Syscall/filesystem/network evidence is collected by a `TelemetryBackend` (`src/behaviour/sandbox.rs`), currently `strace` by default and always available as a fallback. `--telemetry-backend auto|strace|ebpf` selects among an optional, lower-overhead eBPF backend: a separately built external helper (`helpers/layerfault-ebpf-telemetry`, deliberately outside the main `forbid(unsafe_code)` crate — Aya's host-side program-loading APIs require `unsafe`) observes a sandbox run from the host side, since Layerfault's seccomp filter denies `bpf()`/`perf_event_open()` to the sandboxed workload itself. The helper streams normalized events over a bounded, versioned, length-prefixed JSON wire protocol (`layerfault-telemetry-protocol`, `src/behaviour/ebpf_telemetry.rs`); every frame is treated as untrusted input (bounded frame/stream size, closed event-type enum, schema-version check, and run-token/PID-namespace scope re-validation) regardless of what the helper claims to have already filtered.

Before Layerfault will trust the helper it hash- and version-verifies it against a compile-time-embedded manifest (`helpers/layerfault-ebpf-telemetry/EXPECTED.toml`, checked in `src/behaviour/ebpf_verify.rs`) — the first pinned-identity check in the codebase, closest in spirit to (but stricter than) the discovery-to-launch TOCTOU re-hash `sigstore.rs` performs for `cosign`. `--telemetry-backend ebpf` fails closed (hard error, no fallback) if the helper is missing, unverified, or version-mismatched; `--telemetry-backend auto` falls back to strace in the same situations but always records why via `SandboxTelemetry.backend_degraded`, never silently. `layerfault doctor` reports current eBPF helper availability under the `ebpf-telemetry` check.

As of this writing, hash/version verification and the wire protocol are real and enforced. The probe object (`helpers/layerfault-ebpf-telemetry-ebpf`, a standalone `no_std` crate built via `bpf-linker` against `bpfel-unknown-none`/`bpfeb-unknown-none`) compiles to a real, verified eBPF ELF object with working `on_exec`/`on_exit` tracepoint programs and a ring buffer, and the host helper's ring-buffer-consumption loop normalizes those into wire-protocol frames. The `on_connect`/`on_openat`/`on_unlinkat`/`on_renameat2` programs are not yet implemented — reading syscall arguments at those tracepoints correctly requires offsets from each event's live kernel `format` file, which needs a real root/debugfs-capable test host to generate and verify against. Kernel *attachment* of the exec/exit programs is unverified in this codebase's development environment for the same reason (no root, `unprivileged_bpf_disabled=2`): `helpers/layerfault-ebpf-telemetry/tests/load_real_object.rs` runs the real host binary against the real compiled object and confirms the expected outcome — a clean `Operation not permitted (os error 1)` at map creation, not a panic or hang — but cannot confirm a successful attach without a root/CAP_BPF-capable host. `--telemetry-backend ebpf`/`auto` therefore still always resolve to strace today (`LIVE_EBPF_COLLECTION_IMPLEMENTED = false` in `src/behaviour/telemetry_backend.rs`), pending that verification and a release pipeline populating `EXPECTED.toml`'s real hash. The default remains `strace`/`auto`-without-eBPF until a parity/performance corpus against strace justifies otherwise.
...
`intelligence` owns signed data-only knowledge; `runtime_security` owns runtime discovery, posture, exploitability, and compatibility; `model/{declarative,tokenizer,identity,forensics,lineage,research}` owns model analysis; `inventory` owns passports and persistent drift state; and `admission` owns signed execution binding. Policy consumes typed evidence from these domains. Framework mappings are attached after detection and never act as detector inputs.
