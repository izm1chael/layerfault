...
| 6 | Scan incomplete: a wall-clock deadline or cancellation interrupted the scan before it finished (`layerfault pipeline` on a directory/package target only) |
...
`layerfault pipeline` on a directory/package target reports exit `6` and JSON `"decision":"INCOMPLETE"` when a wall-clock deadline or cancellation interrupts the scan before it finishes and no blocking/policy-block finding was already established from the bytes examined so far (see `SecurityDecision::escalate_for_control_interruption`). This is deliberately distinct from `BLOCK` (exit `3`/`4`): it means "we didn't finish checking in time," not "we found something dangerous," and `summary.blocking` in the JSON output stays `0` in this case. If the same bytes already produced an integrity failure, scanner `Fail`, or policy block before the interruption, that finding wins and the process still returns `2`, `3`, or `4` — an interrupted scan can never *lower* a verdict already established from evidence actually examined. Automation should treat exit `6` as "retry with a larger `--budget-profile` or `--timeout-seconds`," not as a security block.
